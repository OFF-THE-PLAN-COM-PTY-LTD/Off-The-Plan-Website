...
| 7 | ✅ **Done 2026-07-02** | **`frontage_m` and `depth_m` columns are integer** | Fixed via SQL migration only — the client input already has `type=number` (accepts decimals) and the server does `Number()` (accepts decimals). Only the DB column type was blocking. Same fix pattern as `bath` earlier today. Ched ran the ALTER migration. | Done |
