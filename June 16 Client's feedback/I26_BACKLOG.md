...
| 8 | ✅ **Done 2026-07-02** | **Optional Uploads gap** | Ched compared legacy vs ours — layouts match except legacy has an "Upload Additional Video" URL field between "Upload floor plans" and "3D Tour Link". Added the missing field (uses the pre-existing `additional_video_url` DB column and `additionalVideoUrl` state variable — everything was wired except the UI input). Same +Add / Save button shape as 3D Tour Link. | Done |
| 9 | ⏭️ **Skipped 2026-07-02** | **Property Features preset list missing common items** | Ched checked legacy — same preset list (no "pet-friendly building" etc.). The "Add your own" fallback covers any missing amenity a developer needs. Matches legacy exactly, no change required. | Skipped |
